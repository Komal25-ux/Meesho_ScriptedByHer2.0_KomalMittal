...
def check_pending_selection(state: SakhiState) -> SakhiState:
    ...
    # A tap now arrives wrapped in a Product Card Click trigger phrase (see
    # handleProductCardClick in App.jsx), not the bare product name - strip
    # it back off before the exact-name comparison below. Safe no-op if this
    # message isn't one of those (e.g. the picker was abandoned and the user
    # typed something else instead).
    selection_text = _strip_trigger_prefix(raw_input)
    selected = next(
        (o for o in pending.get("options", []) if (o.get("name") or "").strip().lower() == selection_text.strip().lower()),
        None
    )
...
def detect_intent(state: SakhiState) -> SakhiState:
    ...
    # Deterministic fast-path for the two Product Card Click triggers (see
    # handleProductCardClick in App.jsx / CATALOG_SHARE_TRIGGER_RE above) -
    # skips the LLM classifier entirely. These exact prefixes are only ever
    # sent by that one click handler, never typed by a user, so there's no
    # real ambiguity for the LLM to resolve, and a fixed rupee/product-name
    # detail-heavy string is exactly the kind of input a semantic classifier
    # can misroute for no benefit.
    if CATALOG_SHARE_TRIGGER_RE.match(user_input):
        intent = "CATALOG"
    elif CUSTOMER_DETAILS_TRIGGER_RE.match(user_input):
        intent = "CUSTOMER"
    elif user_input:
...
def run_catalog_agent(state: SakhiState) -> SakhiState:
    ...
    # CRITICAL RULE FOR SHARING (see prompt_catalog_agent.md): a Product Card
    # Click in Reseller mode (handleProductCardClick in App.jsx) always sends
    # this exact trigger phrase wrapping the tapped product's name. Strip it
    # back off to the bare name here so the exact-name lookup below matches
    # (and so the reply below can be tailored to explicitly offer "post now
    # or change the price" - see is_share_trigger further down).
    is_share_trigger = bool(CATALOG_SHARE_TRIGGER_RE.match(user_input))
    if is_share_trigger:
        user_input = _strip_trigger_prefix(user_input)
    ...
    if similar_skus:
        ...
        if is_share_trigger:
            # Group Sharing flow (see prompt_catalog_agent.md) - explicitly
            # acknowledges the selected item, states the suggested price
            # (Python-sourced cost + margin, never invented), and asks
            # permission to post OR offers a price change in the same
            # message. "Yes"/a new price on the next turn is resolved by the
            # existing check_pending_approval/_extract_price flow - no
            # separate state machine needed for this trigger.
            reply_text = (
                f"Zaroor Didi! *{matched_product.get('name')}* ki cost price ₹{cost} hai. "
                f"Hum isko WhatsApp group mein ₹{selling_price} mein post kar sakte hain "
                f"(aapka profit ₹{selling_price - cost} hoga).\n\n"
                f"{caption_response.ui_text}\n\n"
                f"Kya main isko post kar doon, ya aap price change karna chahengi?"
            )
            reply_tts_text = (
                f"ज़रूर दीदी! {caption_response.product_name_tts} की कॉस्ट प्राइस {cost} रुपये है। "
                f"हम इसको व्हाट्सएप ग्रुप में {selling_price} रुपये में पोस्ट कर सकते हैं, "
                f"आपका प्रॉफिट {selling_price - cost} रुपये होगा।\n\n"
                f"{caption_response.tts_text}\n\n"
                f"क्या मैं इसको पोस्ट कर दूं, या आप प्राइस चेंज करना चाहेंगी?"
            )
        else:
            reply_text = (
...
def run_customer_agent(state: SakhiState) -> SakhiState:
    ...
    # CRITICAL RULE FOR ITEM CLICKS (see prompt_customer_agent.md): a Product
    # Card Click in Customer mode (handleProductCardClick in App.jsx) always
    # sends this exact trigger phrase wrapping the tapped product's name.
    # Strip it back off to the bare name so the exact-name lookup below
    # matches; is_details_trigger is passed into the prompt further down as
    # an explicit PRODUCT_CARD_CLICK signal (Priority 1.5) so the model
    # treats this as high purchase interest rather than a generic query.
    is_details_trigger = bool(CUSTOMER_DETAILS_TRIGGER_RE.match(user_input))
    if is_details_trigger:
        user_input = _strip_trigger_prefix(user_input)
